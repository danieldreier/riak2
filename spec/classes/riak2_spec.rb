    ...
    on_supported_os.each do |os, facts|
      context "on #{os}" do
        ...
        context "riak2 class without any parameters" do
          ...
          it { is_expected.to contain_class('riak2::install') }
        ...
        context "riak2 class with repositories disabled" do
          let(:params) {{
            :manage_repo => false
          }}
          ...
          case facts[:osfamily]
          when 'RedHat'
            it { is_expected.to_not contain_class('riak2::repository::el') }
            it { is_expected.to_not contain_yumrepo('basho_riak') }
          when 'Debian'
            it { is_expected.to_not contain_class('riak2::repository::debian') }
            it { is_expected.to_not contain_apt__source('riak') }
          end
        end
        context "riak2 class with package set to specific version" do
          let(:params) {{
            :version => '2.0.5'
          }}
          it_behaves_like "class required behavior"
        ...
        context "riak2 class with package install disabled" do
          let(:params) {{
            :manage_package => false
          }}
          it_behaves_like "class required behavior"
          it { is_expected.to_not contain_package('riak') }
          it { is_expected.to_not contain_class('riak2::install') }
        end
        context "riak2 class with custom package name" do
          let(:params) {{
        ...
        context "riak2 class with custom service name" do
          let(:params) {{
        ...
        context "riak2 class with custom config settings" do
          let(:params) {{
            :settings => {
              'foo'    => 'bar',
              'dtrace' => 'on',
